...
def test_validate_models_prefixed_by_provider():
    ...
    # Valid model strings
    assert validate_models_prefixed_by_provider(["openai:gpt-4o-mini"]) is True
    assert validate_models_prefixed_by_provider(["anthropic:claude-3-5-haiku"]) is True
    assert validate_models_prefixed_by_provider(["o:gpt-4o-mini", "a:claude-3-5-haiku"]) is True
    assert validate_models_prefixed_by_provider(["gateway:glm-4.7", "oc:qwen3-max"]) is True
...
def test_validate_provider():
    ...
    assert validate_provider("o") is True
    assert validate_provider("a") is True
    assert validate_provider("gateway") is True
    assert validate_provider("oc") is True
